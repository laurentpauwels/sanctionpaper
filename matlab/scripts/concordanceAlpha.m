...
% (approxEffectSanction.m) in the Imbs and Pauwels (2023, EconPolicy). 
% It depends on master script evaluateTradeSanction.m. 
% It requires to load WIOD Socio-Economic Accounts 2016 release.
% Refer to the Readme.md file for more details.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Concordance between WIOD16 and ICIO21, output = alpha_r
...
N = length(sea16_text.countrycode);
R = length(sea16_text.industrycode);

% Averaging over time and reshaping RxN
pvabar = reshape(squeeze(mean(VA_natcur,2, 'omitnan')),R,N); %PVA in millions of Nat Cur.
...
clearvars alpha_* alpha pvabar compbar R N -except alpha_r
